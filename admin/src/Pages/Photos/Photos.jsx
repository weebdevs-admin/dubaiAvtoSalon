...
function Photos() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://172.20.10.2:4100/gallery/create', { img: selectedFile.name }, {
        headers: {
  ...
  const fetchImages = async () => {
    try {
      const response = await axios.get('http://172.20.10.2:4100/gallery');
  ...
  const handleDelete = async (image) => {
    ...
    try {
      ...
      await axios.delete(`http://172.20.10.2:4100/gallery/delete/${image._id}`);
