...
function News() {
  ...
  const fetchImages = async () => {
    try {
      const response = await axios.get('http://172.20.10.2:4100/news');
  ...
  const handleDelete = async (newsItem) => {
    try {
      await axios.delete(`http://172.20.10.2:4100/news/delete/${newsItem._id}`);
    ...
    if (formData.title !== '...' || formData.desc !== '...') {
      try {
        if (editingItemId) {
          ...
          await axios.put(`http://172.20.10.2:4100/news/update/${editingItemId}`, formData);
          toast.success('Information updated successfully');
        } else {
          // If editingItemId doesn't exist, create a new item
          await axios.post('http://172.20.10.2:4100/news/create', formData);
